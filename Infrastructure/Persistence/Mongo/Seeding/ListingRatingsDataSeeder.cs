...
internal sealed class ListingRatingsDataSeeder(IMongoDatabase database) : IDataSeeder
{
    public string Name => "listing_ratings";

    public async Task SeedAsync(CancellationToken ct = default)
    {
        var docs = SeedJsonLoader.LoadArrayFromResource("listing_ratings.json");
        ...
        foreach (var d in docs)
        {
            ...
            var filter = Builders<BsonDocument>.Filter.Eq("listingId", d.GetValue("listingId").AsInt32);
